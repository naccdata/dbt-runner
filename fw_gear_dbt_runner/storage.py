...
class StorageManager:
    ...
    def __init__(self, api_key: str, storage_label: str):
        ...
        self.client = FWClient(api_key=api_key)
        self.storage_client = None
        self._initialize_storage()

    def _initialize_storage(self) -> None:
        """Initialize storage client by fetching credentials from Flywheel API."""
        log.info(f"Initializing storage client for: {self.storage_label}")

        # Get list of available storages
        storages_response = self.client.get("/xfer/storages")
        storages = [
            s for s in storages_response["results"] if s["label"] == self.storage_label
        ]

        if not storages:
            raise ValueError(
                f"Storage with label '{self.storage_label}' not found. "
                f"Available storages: {[s['label'] for s in storages_response['results']]}"
            )
        ...
        storage_creds = self.client.get(f"/xfer/storage-creds/{storage['_id']}")
        storage_url = storage_creds["url"]

        # Create storage client
        self.storage_client = create_storage_client(storage_url)
    ...
    def download_dataset(self, source_prefix: str, local_dir: Path) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to list files at '{source_prefix}': {str(e)}"
            ) from e

        if not files:
            raise RuntimeError(f"No files found at source prefix: {source_prefix}")
        ...
        for file_info in files:
            ...
            try:
                with self.storage_client.get(file_info.path) as remote_file:
                    with open(local_file_path, "wb") as local_file:
                        local_file.write(remote_file.read())
            except Exception as e:
                raise RuntimeError(
                    f"Failed to download '{file_info.path}': {str(e)}"
                ) from e

        log.info(f"Dataset downloaded successfully to: {local_dir}")

    def upload_results(
        self, local_dir: Path, output_prefix: str, exclude_patterns: List[str] = None
    ) -> None:
    ...
    def upload_file(
        self, local_file: Path, output_prefix: str, relative_path: str = None
    ) -> None:
        ...
        if not local_file.is_file():
            raise RuntimeError(f"Path is not a file: {local_file}")
        ...
        try:
            ...
            raise RuntimeError(f"Failed to upload '{local_file}': {str(e)}") from e
    ...
    def verify_access(self, prefix: str) -> bool:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to verify access to '{prefix}': {str(e)}"
            ) from e
